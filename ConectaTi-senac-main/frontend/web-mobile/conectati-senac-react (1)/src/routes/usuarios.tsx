...
function Usuarios() {
  ...
  const alterarAtivo = async (usuario: Usuario) => {
    const acao = usuario.ativo ? "Desativar" : "Ativar";
    if (!confirm(`${acao} este usuario?`)) return;
    try {
      await usuariosService.atualizar(usuario.id, { ...usuario, ativo: !usuario.ativo });
      carregar();
    } catch (error) {
      setErro(
        error instanceof Error
          ? error.message
          : `Nao foi possivel ${acao.toLowerCase()} o usuario.`,
      );
    }
  };
    ...
    <main className="content">
      <header className="topbar">
        <div>
          ...
          <p className="muted">Dados fornecidos pela API.</p>
        </div>
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id}>
                  ...
                  <td>
                    <button className="btn btn-sm btn-ghost" onClick={() => alterarAtivo(item)}>
            ...
            <form className="form" onSubmit={salvar}>
              ...
              <label>
                ...
                <select
                  value={form.tipo}
                  onChange={(event) =>
                    setForm({ ...form, tipo: event.target.value as TipoUsuario })
                  }
                >
