...
export const Route = createFileRoute("/novo-chamado")({
  component: () => (
    <AppShell active="novo-chamado">
      <NovoChamado />
    </AppShell>
  ),
});
...
function NovoChamado() {
  ...
  useEffect(() => {
    categoriasService
      .listar()
      .then(setCategorias)
      .catch((error) => setAlerta(error.message));
    salasService
      .listar()
      .then(setSalas)
      .catch((error) => setAlerta(error.message));
    ...
    <main className="content">
      <header className="topbar">
        <div>
          ...
          <p className="muted">Descreva sua solicitacao.</p>
        </div>
      </header>
      <section className="card reveal visible">
        <form className="form form-grid" onSubmit={submit}>
          ...
          <label>
            ...
            <select
              value={form.salaId}
              onChange={(event) => setForm({ ...form, salaId: event.target.value })}
            >
          ...
          {alerta && <div className="alerta sucesso full">{alerta}</div>}
          <div className="full actions-row">
            <button className="btn btn-primary">Enviar chamado</button>
          </div>
        </form>
      </section>
    </main>
