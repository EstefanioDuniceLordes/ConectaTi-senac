...
const SALAS_BASE = [
  { nome: "Sala 01", tipo: "Sala" },
  { nome: "Sala 02", tipo: "Sala" },
  { nome: "Sala 03", tipo: "Sala" },
  { nome: "Sala 04", tipo: "Sala" },
  { nome: "Sala 05", tipo: "Sala" },
  { nome: "Sala 08", tipo: "Sala" },
  { nome: "Sala 12", tipo: "Sala" },
  { nome: "Sala 15", tipo: "Sala" },
...
const USUARIOS_BASE = [
  {
    id: "u1",
    nome: "Rayssa Paiva",
    email: "rayssa@senacdf.com.br",
    perfil: "Instrutor",
    unidade: "Senac Taguatinga Sul - Taguatinga/DF",
  },
  {
    id: "u2",
    nome: "Carla Mendes",
    email: "carla@senacdf.com.br",
    perfil: "Coordenação",
    unidade: "Senac Taguatinga Sul - Taguatinga/DF",
  },
  {
  ...
  },
  {
    ...
    email: "ana.coord@senacdf.com.br",
    perfil: "Coordenação",
    unidade: "Senac Taguatinga Sul - Taguatinga/DF",
  },
  {
...
const PERMISSOES = {
  Instrutor: {
    menu: ["home", "chamados", "novo-chamado", "equipamentos"],
    verChamados: "proprios",
    acoes: false,
    novoChamado: true,
    cor: "#2b8cff",
    descricao: "Solicita salas e abre chamados.",
  },
  "Suporte TI": {
    menu: ["home", "dashboard", "chamados", "equipamentos", "relatorios"],
    verChamados: "todos",
    acoes: true,
    novoChamado: false,
    cor: "#ff8a3d",
    descricao: "Atende chamados e gerencia equipamentos.",
  },
  Coordenação: {
    menu: ["home", "dashboard", "chamados", "equipamentos", "relatorios"],
    verChamados: "todos",
    ...
    novoChamado: false,
    cor: "#8b5cf6",
    descricao: "Acompanha indicadores e chamados.",
  },
  Administrador: {
    menu: ["home", "dashboard", "equipamentos", "usuarios", "unidades"],
    verChamados: "todos",
    acoes: true,
    novoChamado: false,
    cor: "#16a34a",
    descricao: "Controle total: usuários, unidades e equipamentos.",
  },
};
function getPerfil() {
  return localStorage.getItem("conectati_perfil") || "";
}
function getPermissoes() {
  return PERMISSOES[getPerfil()] || null;
}

function paginaInicialPerfil(perm) {
  const primeiros = ["dashboard", "mapa-salas", "chamados", "equipamentos"];
  for (const p of primeiros) {
    if (perm.menu.includes(p)) return p;
  }
...
function setupLogin() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    setTimeout(() => {
      window.location.href = paginaInicialPerfil(perm) + ".html";
    }, 1000);
  });
}
...
function setupChamados() {
  ...
  const perfil = getPerfil();
  const base =
    perm && perm.verChamados === "proprios"
      ? CHAMADOS.filter((c) => c.perfil === perfil)
      : CHAMADOS;
  renderChamados(base);
  ...
  function aplicar() {
    ...
    const filtrados = base.filter((c) => {
      return (
        (!q || c.solicitante.toLowerCase().includes(q)) &&
...
function setupNovoChamado() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    alerta.textContent =
      "Chamado enviado com sucesso! Nº #" + String(Math.floor(Math.random() * 900) + 100);
    alerta.classList.remove("hidden");
    form.reset();
    document.getElementById("data").value = hoje.toISOString().slice(0, 10);
    document.getElementById("hora").value = hoje.toTimeString().slice(0, 5);
...
function acaoRelatorio(nome) {
  toast(`${nome} — ação simulada`);
}
...
function getSalas() {
  return ensureSeed(
    "conectati_salas",
    SALAS_BASE.map((s) => ({ ...s, chave: null /* {responsavel,motivo,desde} */ })),
  );
}
function setSalas(v) {
  writeStore("conectati_salas", v);
}

function getUsuarios() {
  return ensureSeed("conectati_usuarios", USUARIOS_BASE);
}
function setUsuarios(v) {
  writeStore("conectati_usuarios", v);
}

function getUnidades() {
  return ensureSeed("conectati_unidades", UNIDADES_BASE);
}
function setUnidades(v) {
  writeStore("conectati_unidades", v);
}

function getSolicitacoes() {
  return ensureSeed("conectati_solicitacoes", []);
}
function setSolicitacoes(v) {
  writeStore("conectati_solicitacoes", v);
}
...
function setupMinhasTurmas() {
  ...
  if (sel) {
    ...
    sel.innerHTML = salas
      .map((s) => `<option value="${s.nome}">${s.nome} (${s.tipo})</option>`)
      .join("");
  }
  ...
  document.getElementById("form-solicitar")?.addEventListener("submit", (e) => {
    ...
    const nova = {
      ...
      instrutorNome:
        getUsuarios().find((u) => u.email.toLowerCase() === email)?.nome || "Instrutor",
...
function popularResponsaveis() {
  ...
  sel.innerHTML = alvos
    .map((u) => `<option value="${u.id}">${u.nome} — ${u.perfil}</option>`)
    .join("");
}
function renderMapa() {
  const wrap = document.getElementById("mapa-salas");
...
function setupUsuarios() {
  ...
  function fillUnidades() {
    ...
    unidadesSel.innerHTML = unis
      .map((u) => `<option value="${u.nome} - ${u.cidade}">${u.nome} - ${u.cidade}</option>`)
      .join("");
  }
  fillUnidades();
  ...
  function render() {
   ...
   `;
    ...
    tbody
      .querySelectorAll("button[data-editar]")
      .forEach((b) => b.addEventListener("click", () => abrir(b.dataset.editar)));
